...
if (isset($_POST['update'])) {
  ...
  echo '<script type="text/javascript"> alert("User ' . $username . ' is disapproved! It will go to Archive List.")</script>';
    ...
    $query_army_user = "INSERT INTO army_users VALUES('','','$firstname','','$lastname','$username','$email','$password','$user_type','$rank_id','','','$school_id','$up_status','$up_user_status','$date_modified','$time_modified','$date_modified','$time_modified')";
      ...
      $result_get_id = mysqli_query($conn, "SELECT * FROM army_users WHERE username ='$username'");
      ...
      // Inserting user agreement data 
        mysqli_query($conn, "update agreement set army_id = '$army_id' where army_id = '$sch_id'") or die("Query 4 is incorrect....");

      // Delete the registration process of school coordinator
