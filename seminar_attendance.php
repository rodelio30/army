<table id="example" class="table table-hover my-0" style="width:100%">
    ...
    <tbody>
        ...
        if ($seminar_attendance_counter > 0) {
                $result = mysqli_query($conn, "select att_id, seminar_id, user_id, date_attended, time_attended, status from seminar_attendance WHERE seminar_id = $seminars_id  && status != 'Declined' ORDER BY att_id") or die("Query for seminar Attendance Error....");
